...
| 指標 | 本工具 | AEDT Circuit | 差異 |
| --- | ---: | ---: | ---: |
| 阻抗凹陷深度 | 50.4 Ω | 49.7 Ω | **1.4%** |
| 凹陷位置 | 對齊後重合 | — | — |
| 回穩差分阻抗 | 73.3 Ω | 86.0 Ω | 12.7 Ω |

**凹陷是重點**：響應用的是 |Γ| = ΔZ/2Z，量的是阻抗階梯的落差而不是
絕對位準，而落差兩邊只差 1.4%。

還有第三條獨立佐證：AEDT 的回穩值 86.0 Ω 與**頻域 Sdd11 在 0.45 GHz
直接代數推得的 85.5 Ω** 相符。這兩條路徑不共用任何程式碼，對得上代表
不是同一個錯誤算了兩遍。

回穩位準那 12.7 Ω 的差距尚未收斂，這裡照實列出來而不是裁掉——已排除
Touchstone 的 DC 列外推（拿掉重算，14 個設計的 |Γ| 一個都沒變、名次
完全不動）。要對外引用**絕對**差分阻抗之前得先解決它；用於最佳化的
排序不受影響。
