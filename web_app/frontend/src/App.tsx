            ...
            <>
              ...
              {designs.length > 0 && (
                <div className="glass-panel" style={{ maxHeight: 340, overflowY: "auto" }}>
                  ...
                  {/*
                    前緣上每一點在畫面上長得一樣可信，但實測不是。理由是
                    **穩定性**不是準確度：每補一個真解點重新擬合，端點的
                    keep-out 面積在 6.80 / 16.55 / 6.62 mm² 之間擺盪 2.4 倍，
                    膝點只動 12%（5.42 / 6.06 / 5.60）。端點由某一端的極值
                    決定，會被雜訊推著跑；膝點由前緣的形狀決定，不會。
                    所以這裡要把「該拿哪一點」講出來，不能只染一種藍色。
                  */}
                  <div className="teach-note" style={{ marginBottom: 10 }}>
                    <b>建議拿膝點跟客戶談。</b>前緣上的 |Γ| 是 MOP
                    的預測值。每補一個真解點重新擬合，端點的面積實測擺盪
                    2.4 倍，膝點只動 12%——端點由極值決定會被雜訊推著跑，
                    膝點由前緣形狀決定則不會。
